...
The monitoring runs every 3 hours, checks all four environments sequentially, detects configuration changes, and sends email alerts with detailed change summaries.

Each workflow run checks all four environments one at a time, reducing GitHub Actions usage while maintaining comprehensive monitoring.
...
The workflow runs automatically every 3 hours. Each run checks all four environments sequentially:
- dev
- jkc-uat
- jkc-prod
- prod
...
4. Expand job to view detailed logs

**Change Logs:**
1. Go to the workflow run page
2. Scroll to Artifacts section
3. Download change_log_{environment}.json files
...
**Polling Strategy:**
```
Polls per run: 1 (with 1 retry on failure)
If successful: Exit immediately
If failed: Retry once, then exit
Total time per run: ~20 seconds
```
